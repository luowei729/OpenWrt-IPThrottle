...
### Fixed
- 北京时间 2026-06-10 18:05 - 修复守护进程在无启用规则时死循环
  - **问题**: 没有启用规则时 nftables 表不会创建，守护进程检测到表丢失后不断尝试重建
  - **修复**: 检测 nftables 表前先检查是否有启用的规则，无规则时不尝试重建
  - **日志**: 避免 "nftables table lost, recreating rules..." 无限循环
...
- 北京时间 2026-06-10 14:15 - 修复 OpenWrt 25 全新安装报错
  ...
  - **问题2: 包管理器数据库锁冲突** `Unable to lock database` / `Could not lock opkg.lock`
    - 原因: 包安装后 procd 自动启动服务，此时包管理器(opkg/apk)数据库仍被锁
    - 修复1: `init.d/ipthrottle` 添加 `wait_pkg_lock()` 函数，启动前等待锁释放（最长60秒）
      - 同时支持 opkg (`/var/lock/opkg.lock`) 和 apk (`/lib/apk/db/lock`)
    - 修复2: `deps.sh` 的 `install_package()` 添加重试机制
      - opkg: 检测到锁冲突时自动重试（最多10次，间隔3秒）
      - apk: 检测到锁冲突时自动重试（最多10次，间隔3秒）
