...
| **配置存储** | UCI | /etc/config/ipthrottle |
...
parse_rules() {
    local mark=1
    for rule in $(uci -q show ipthrottle | grep "=rule" | sort -t= -k2 -n); do
        local enabled=$(uci -q get ipthrottle.$rule.enabled)
        [ "$enabled" = "1" ] || continue
...
│   ├── etc/
│   │   ├── config/
│   │   │   └── ipthrottle               # UCI 默认配置 (空模板)
│   │   ├── init.d/
│   │   │   └── ipthrottle               # PROCD 服务脚本 (start/stop/restart/reload)
│   │   ├── hotplug.d/
│   │   │   └── iface/
│   │   │       └── 90-ipthrottle        # 网络接口 up/down 事件处理
│   │   └── cron.d/
│   │       └── ipthrottle               # 每分钟 schedule 评估任务
│   └── usr/
│       ├── lib/
│       │   └── ipthrottle/
...
│       └── sbin/
│           └── ipthrottle               # CLI 入口脚本 (调用 core.sh)
├── htdoc/
│   └── luci-static/
│       └── resources/
│           └── view/
...
└── root/
    └── usr/
        ...
        │   ├── luci/
        │   │   └── menu.d/
        │   │       └── luci-app-ipthrottle.json  # LuCI 菜单注册
        │   └── rpcd/
        │       └── acl.d/
        │           └── luci-app-ipthrottle.json  # LuCI ACL 权限
        └── libexec/
            └── rpcd/
                └── ipthrottle           # UBUS RPC 后端 (可选, 用于动态数据查询)
```
...
### 阶段 1: 基础框架 (预计 2h)
- [ ] 创建 package 目录结构
- [ ] 编写 Makefile (PKG_NAME, PKG_VERSION, PKG_RELEASE, Build/Compile, Package/ipthrottle)
- [ ] UCI 默认配置 (/etc/config/ipthrottle 空模板)
- [ ] PROCD init 脚本骨架 (start_service, stop_service, reload_service)
- [ ] 验证: `/etc/init.d/ipthrottle start` 能启动，`ps | grep ipthrottle` 能看到 PROCD 进程
...
- [ ] 自定义 nftables 表/链 (ipthrottle forward/ingress)
...
### 阶段 4: UCI 集成 (预计 2h)
- [ ] 读取 /etc/config/ipthrottle (uci -q show ipthrottle)
...
- [ ] 监听 iface up/down 事件 (hotplug.d/iface/90-ipthrottle)
...
ip_range_expand() {
    ...
    if [ $start_int -gt $end_int ]; then
        logger -t ipthrottle "ERROR: IP range start > end: $range"
        return 1
    fi
...
generate_nft_rules() {
    ...
    for rule in $rules; do
        ...
        local name=$(uci -q get ipthrottle.$rule.name)
        local ip_entries=$(uci -q get ipthrottle.$rule.ip_entry)
...
check_and_reload() {
    ...
    local rules=$(uci -q show ipthrottle | grep "=rule")
    for rule in $rules; do
        local schedule_type=$(uci -q get ipthrottle.$rule.schedule_type)
        local schedule_json=$(uci -q get ipthrottle.$rule.schedule_json)
        
        if [ "$schedule_type" = "weekly" ]; then
            if schedule_active "$schedule_json"; then
                # 检查上次状态
                local last_status=$(cat /tmp/ipthrottle_$rule.status 2>/dev/null)
                if [ "$last_status" != "active" ]; then
                    need_reload=1
                    echo "active" > /tmp/ipthrottle_$rule.status
                fi
            else
                local last_status=$(cat /tmp/ipthrottle_$rule.status 2>/dev/null)
                if [ "$last_status" = "active" ]; then
                    need_reload=1
                    echo "inactive" > /tmp/ipthrottle_$rule.status
                fi
            fi
        fi
    done
...
| 场景 | 处理方式 |
|------|---------|
| UCI 配置字段缺失/非法 | 跳过该规则，log 警告 (`logger -t ipthrottle "WARN: ..."`), 继续加载其他规则 |
...
reload_service() {
    # 1. 生成新规则到临时表
    nft -f /tmp/ipthrottle_new.nft || return 1
    
    # 2. 原子替换
    nft replace table ip ipthrottle /tmp/ipthrottle_new.nft
    
    # 3. 清理临时文件
    rm -f /tmp/ipthrottle_new.nft
}
```
